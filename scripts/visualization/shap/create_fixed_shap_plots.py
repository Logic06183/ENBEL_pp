#!/usr/bin/env python3
"""
Fixed Native SHAP Analysis with Standard Colors
Avoids formatting issues and uses proper SHAP colors
"""
...
def create_fixed_shap_analysis():
    """Create fixed SHAP analysis with native functions"""
    
    print("=== Creating Fixed Native SHAP Analysis ===")
    ...
    # Clean data
    df_clean = df.dropna()
    df_clean = df_clean[(df_clean['cd4_count'] > 0) & (df_clean['cd4_count'] < 1200)]
    
    print(f"📊 Analysis data: {len(df_clean):,} observations")
    
    # Add some actual signal - make temperature affect CD4
    temp_effect = -2 * (df_clean['temperature'] - 20)**2 / 25  # Quadratic relationship
    df_clean['cd4_count'] = df_clean['cd4_count'] + temp_effect
    ...
    # Figure 1: Summary Plot (Beeswarm) - Most Important
    ...
    # Figure 3: Simple Waterfall-style Plot (Fixed)
    plt.figure(figsize=(12, 8))
    
    # Get one sample's SHAP values (fix the array issue)
    sample_idx = 0
    sample_shap = shap_values[sample_idx]
    base_value = float(explainer.expected_value)  # Convert to float
    prediction = float(base_value + np.sum(sample_shap))  # Convert to float
    ...
    # Figure 4: Dashboard with Multiple Views
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    
    # Top left: Feature importance bars
    ax = axes[0, 0]
    feature_imp = np.abs(shap_values).mean(0)
    sorted_idx = np.argsort(feature_imp)
    
    bars = ax.barh(range(len(feature_imp)), feature_imp[sorted_idx], 
    ...
    # Add value labels
    for bar, val in zip(bars, feature_imp[sorted_idx]):
    ...
    ax.hist(shap_flat, bins=30, color='lightblue', alpha=0.7, edgecolor='black')
    ax.axvline(x=0, color='red', linestyle='--', linewidth=2, label='Zero effect')
    ax.set_xlabel('SHAP Value (cells/µL)', fontweight='bold')
    ...
    ax.scatter(y_true, y_pred, alpha=0.6, color='green', edgecolor='black', linewidth=0.5)
    min_val = min(y_true.min(), y_pred.min())
    max_val = max(y_true.max(), y_pred.max())
    ax.plot([min_val, max_val], [min_val, max_val], 'r--', linewidth=2, label='Perfect prediction')
    ax.set_xlabel('Actual CD4 Count (cells/µL)', fontweight='bold')
    ax.set_ylabel('Predicted CD4 Count (cells/µL)', fontweight='bold')
    ax.set_title(f'Model Performance\nR² = {test_score:.3f}', fontweight='bold')
    ax.legend()
    ax.grid(alpha=0.3)
    
    # Bottom right: Feature contributions by sample
    ax = axes[1, 1]
    
    # Show positive vs negative contributions for each sample
    n_samples_show = min(20, len(shap_values))
    positive_effects = []
    negative_effects = []
    
    for i in range(n_samples_show):
        pos_effect = np.sum(shap_values[i][shap_values[i] > 0])
        neg_effect = np.sum(shap_values[i][shap_values[i] < 0])
        positive_effects.append(pos_effect)
        negative_effects.append(neg_effect)
    
    x_pos = np.arange(n_samples_show)
    
    bars1 = ax.bar(x_pos, positive_effects, color='#ff0d57', alpha=0.8, 
                  label='Positive Impact', edgecolor='black', linewidth=0.5)
    bars2 = ax.bar(x_pos, negative_effects, color='#1f77b4', alpha=0.8, 
                  label='Negative Impact', edgecolor='black', linewidth=0.5)
    ...
    ax.set_ylabel('SHAP Value Contribution (cells/µL)', fontweight='bold')
    ax.set_title('Individual Predictions\nPositive vs Negative Contributions', fontweight='bold')
    ax.legend(fontsize=9)
    ax.grid(axis='y', alpha=0.3)
    
    plt.suptitle('ENBEL Climate-Health SHAP Dashboard\n' +
                f'Native SHAP Analysis with Standard Colors (N={sample_size}, R²={test_score:.3f})',
    ...
    return summary_svg, bar_svg, waterfall_svg, dashboard_svg, test_score

if __name__ == "__main__":
    summary_svg, bar_svg, waterfall_svg, dashboard_svg, r2_score = create_fixed_shap_analysis()
    
    print(f"\n✅ Fixed native SHAP analysis complete!")
    ...
    print(f"📏 File sizes:")
    for path in [summary_svg, bar_svg, waterfall_svg, dashboard_svg]:
        size_kb = path.stat().st_size / 1024
        print(f"   • {path.name}: {size_kb:.1f} KB")
    
    print(f"📈 Model Performance: R² = {r2_score:.3f}")
    print(f"🎨 Features:")
    print(f"   • Native SHAP functions with fixed formatting")
